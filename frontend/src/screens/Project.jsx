...
const Project = () => {
  ...
  const {
    ...
    loading: scenLoading,
    error: scenError,
  } = useScenarios(projectId, project?.is_demo);
  ...
  if (scenarios && scenarios.length != 0 && !scenariosInitialized) {
    ...
    const preselected =
      location.state?.selectedScenarioId &&
      ...
      <main className="page-container flex-1 flex flex-col w-full">
        ...
        <div className="main-content flex-1 flex flex-col">
          <div className="flex flex-col gap-4 my-4">
            <h3>
              {projectCountryCode
                ? `${projectLocation}, ${projectCountryCode}`
                : projectLocation}
            </h3>
            <div className="isolate">
              <Map
                lat={projectCoords.lat}
                lon={projectCoords.lon}
          ...
          {scenarios.length != 0 ? (
            selectedScenario ? (
              <div id="report">
                <div
                  id="tabs-section"
                  className={`flex justify-between sticky ${isGuest ? "top-[var(--header-height-guest)]" : "top-[var(--header-height)]"} bg-background z-50`}
                >
                  <Tabs
                      ...
                      <div className="action-buttons flex flex-nowrap gap-2">
                        <Tooltip>
                          <TooltipTrigger
                            render={
                              <Link
                                to={`/projects/${project.id}/scenarios/create`}
                              >
                        ...
                        <Tooltip>
                          <TooltipTrigger
                            render={
                              <Link
                                to={`/projects/${project.id}/scenarios/${selectedScenario.id}`}
                              >
                                <Button size="icon" variant="outline">
                                  <Pencil />
                                </Button>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        <TableRow>
                          ...
                          <TableCell>
                            {selectedScenario.module_amount} modules
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell className="font-semibold">
                          ...
                          <TableCell>
                            {(
                              selectedScenario.installed_power / 1000
                        ...
                        <TableRow>
                          ...
                          <TableCell>
                            {selectedScenario.azimuth}°
                            {` · ${getCompassDirection(selectedScenario.azimuth)}`}
                          </TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
                    <Table>
                      ...
                      <TableBody>
                        <TableRow>
                          ...
                          <TableCell>
                            {selectedScenario.module?.manufacturer}
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell className="font-semibold">Model</TableCell>
                          <TableCell>
                            {selectedScenario.module?.model}
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell className="font-semibold">
                          ...
                          <TableCell>
                            {selectedScenario.module?.nominal_power} Wp
                          </TableCell>
                        </TableRow>
                        <TableRow>
                          <TableCell className="font-semibold">
                          ...
                          <TableCell>
                            {selectedScenario.module?.efficiency}%
                          </TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
                          ...
                          <Card>
                            ...
                            <CardContent className="flex flex-col gap-3">
                              ...
                              <Progress
                                value={Math.min(
                                  100,
                                  (report.specific_yield / report.radiation) *
                                    100,
                                )}
                              />
                              <p className="text-xs text-muted-foreground">
                                This figure represents the system performance
                                relative to the module's rated capacity
                              </p>
                            </CardContent>
                          </Card>
                          <Card>
                            ...
                            <CardContent className="flex flex-col gap-3">
                              ...
                              <Progress
                                value={Math.min(
                                  100,
                                  Math.max(0, report.perf_ratio),
                                )}
                              />
                              <p className="text-xs text-muted-foreground">
                                This figure represents the system performance
                                relative to the location's maximum theoretical
                                output
                              </p>
                            </CardContent>
                          </Card>
