...
def test_update_project_info(auth_client, create_project):
    ...
    response = auth_client.patch(f"/api/projects/{project_id}", json={"name": "newname"})
...
def test_update_project_info_bad_token(client, create_project):
    ...
    response = client.patch(f"/api/projects/{project_id}", headers=bad_headers, json={"name": "newname"})
...
def test_delete_project(auth_client, create_project):
    ...
    project_ids_in_db = [project.get("id") for project in auth_client.get("/api/projects/").json()]

    assert response.status_code == 200
    assert project_id not in project_ids_in_db


def test_delete_project_bad_token(auth_client, create_project):
    project_id = create_project.json().get("id")
    bad_headers = {"Authorization": f"Bearer bad_token"}
    response = auth_client.delete(f"/api/projects/{project_id}", headers=bad_headers)
